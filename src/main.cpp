...
void USART0_printSerial(const char* s) {
	while (*s) {
		USART0_transmit(*s++);
	}
}

// setez culoarea LED-ului RGB
...
// citesc valoarea analogică de la joystick
...
// verific daca un jucator a castigat
...
void loop() {
	...
	if (btn && board[cursorY][cursorX] == ' ') {
		...
		if (checkWin('X')) {
			...
			gameOver = true;
			setRGB(0, 1, 0);
			USART0_printSerial("Jucatorul X a castigat runda!\n");
			winMelody();
			delay(1000);
			resetGame();
			showMenu();
		...
		if (checkWin('O')) {
			...
			setRGB(1, 0, 0);
			lossMelody();
			USART0_printSerial("Jucatorul O a castigat runda!\n");
			delay(1000);
			resetGame();
			showMenu();
